    ...
    while bfsFrontier.empty() == False:        
        ...
        if (state.index(0) > 2):
            ...
            if tuple(neighbour) not in list(explored) and tuple(neighbour) not in list(frontierSet):
                bfsFrontier.put(tuple(neighbour))
                frontierSet.add(tuple(neighbour))
                currentPath.append("Up")
                bfsPath.put(list(currentPath))
        ...
        if maxDepthIndicator:
            print("maxDepth:", maxDepth, "Frontier Size", bfsFrontier.qsize(), "nodesExpanded", nodesExpanded)
